...
async def chat_with_video(
    ...
    results = vectorstore.similarity_search(
        question, k=5, filter={"doc_id": doc_id}
    )
    if not results:
        raise HTTPException(status_code=404, detail="No relevant transcript content found.")

    context = "\n\n".join([r.page_content for r in results])
    system_prompt = (
        "You are a helpful assistant answering questions about a YouTube video. "
        "Use ONLY the transcript excerpts below. If the answer isn't in the transcript, say so.\n\n"
        f"Transcript:\n{context}"
    )
